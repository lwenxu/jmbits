...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class)
	...
	if (!isset($_GET["cmtpage"])) {
		...
		print("<table width=\"97%\" cellspacing=\"0\" cellpadding=\"5\" class='table table-striped'>\n");
		...
		{
			print("<tr><td class=\"embedded-head\" width=\"7%\">".$lang_details['row_download']."</td><td class=\"rowfollow\" width=\"95%\" align=\"left\">");
		...
		$size_info =  "".$lang_details['text_size']."" . mksize($row["size"]);
		$type_info = "&nbsp;&nbsp;&nbsp;".$lang_details['row_type'].":&nbsp;".$row["cat_name"];
		if (isset($row["source_name"]))
			$source_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_source']."&nbsp;".$row[source_name];
		if (isset($row["medium_name"]))
			$medium_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_medium']."&nbsp;".$row[medium_name];
		if (isset($row["codec_name"]))
			$codec_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_codec']."&nbsp;".$row[codec_name];
		if (isset($row["standard_name"]))
			$standard_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_stardard']."&nbsp;".$row[standard_name];
		if (isset($row["processing_name"]))
			$processing_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_processing']."&nbsp;".$row[processing_name];
		if (isset($row["team_name"]))
			$team_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_team']."&nbsp;".$row[team_name];
		if (isset($row["audiocodec_name"]))
			$audiocodec_info = "&nbsp;&nbsp;&nbsp;".$lang_details['text_audio_codec']."&nbsp;".$row[audiocodec_name];
		...
		if ($CURUSER["downloadpos"] != "no")
			$download = "<a title=\"".$lang_details['title_download_torrent']."\" href=\"download.php?id=".$id."\"><span class='icon-download-alt icon-green'></span>&nbsp;<font >".$lang_details['text_download_torrent']."</font></a>&nbsp;|&nbsp;";
		else $download = "";

		tr(
			$lang_details['row_action'], $download. ($owned == 1 ? "<$editlink><span class='icon-reorder icon-green'></span>&nbsp;<font >"           .$lang_details['text_edit_torrent'] . "</font></a>&nbsp;|&nbsp;" : "").  (get_user_class() >= $askreseed_class && $row[seeders] == 0 ? "<a title=\""                    .$lang_details['title_ask_for_reseed']."\" href=\"takereseed.php?reseedid=$id\"><span class='icon-upload-alt icon-green'></span>&nbsp;<font >"
			 .$lang_details['text_ask_for_reseed'] ."</font></a>&nbsp;|&nbsp;" : "") . "<a title=\""
			.$lang_details['title_report_torrent']."\" href=\"report.php?torrent=$id\"><span class='icon-exclamation-sign icon-red'></span>&nbsp;<font >"
			.$lang_details['text_report_torrent']."</font></a>" . $v6button , 1);
		...
		$v6button = "| " .
			...
			"<a href=\"v6player.php?id={$id}&u={$CURUSER['passkey']}\" target=\"_blank\"><b><font > 直接播放</font></b></a>";

		$wybutton = "| " .
			"<script type=\"text/javascript\">function play(){window.location.href='6xvod://{$id}&ty=1&ro=1&id={$id}&ua={$CURUSER['passkey']}';}</script>" .
			"<img height='14px' src=\"pic/v6player.png\"/>" .
			"<a href=\"wyplayer.php?id={$id}&u={$CURUSER['passkey']}\" target=\"_blank\"><b><font > 网页播放</font></b></a>";
		/*********************6vdplayer**************************/
		...
		tr("<a href=\"javascript: klappe_news('descr')\"><span class=\"nowrap\"> ".$lang_details['row_description']."</span></a>", "<div id='kdescr'>".($Advertisement->enable_ad() && $torrentdetailad ? "<div align=\"left\" style=\"margin-bottom: 10px\" id=\"ad_torrentdetail\">".$torrentdetailad[0]."</div>" : "")."<div style='word-wrap:break-word; word-break:break-all;'>".format_comment($row["descr"])."</div></div>", 1);
		}
	...
	if ($imdb_id && $showextinfo['imdb'] == 'yes' && $CURUSER['showimdb'] != 'no')
	{
				...
				case "1" :
					{
						...
						$Cache->add_whole_row();
						print("<tr>");
						print("<td class=\"embedded-head\"><a href=\"javascript: klappe_ext('imdb')\"><span class=\"nowrap\"><img class=\"minus\" src=\"pic/trans.gif\" alt=\"Show/Hide\" id=\"picimdb\" title=\"".$lang_detail['title_show_or_hide']."\" /> ".$lang_details['text_imdb'] . $lang_details['row_info'] ."</span></a><div id=\"posterimdb\">".  $smallth."</div></td>");
						$Cache->end_whole_row();
						$Cache->add_row();
						$Cache->add_part();
		...
		if ($imdb_id)
		{
			...
			if($copies_count > 0)
			{
				...
				$s .= "</table>\n";
				tr("<a href=\"javascript: klappe_news('othercopy')\"><span class=\"nowrap\"><img class=\"".($copies_count > 5 ? "plus" : "minus")."\" src=\"pic/trans.gif\" alt=\"Show/Hide\" id=\"picothercopy\" title=\"".$lang_detail['title_show_or_hide']."\" /> ".$lang_details['row_other_copies']."</span></a>", "".$copies_count.$lang_details['text_other_copies']." <br /><div id='kothercopy' style=\"".($copies_count > 5 ? "display: none;" : "display: block;")."\">".$s."</div>",1);
			}
		}

		if ($row["type"] == "multi")
		{
			$files_info = "".$lang_details['text_num_files']."". $row["numfiles"] . $lang_details['text_files'] . "<br />";
		...
		if ($enablenfo_main=='yes')
			tr($lang_details['row_torrent_info'], "<table><tr>" . ($files_info != "" ? "<td class=\"no_border_wide\">" . $files_info . "</td>" : "") . "<td class=\"no_border_wide\">".$lang_details['row_info_hash'].":&nbsp;".preg_replace_callback('/./s', "hex_esc", hash_pad($row["info_hash"]))."</td>". (get_user_class() >= $torrentstructure_class ? "<td class=\"no_border_wide\">" . $lang_details['text_torrent_structure'] . "<a href=\"torrent_info.php?id=".$id."\">".$lang_details['text_torrent_info_note']."</a></td>" : "") . "</tr></table><span id='filelist'></span>",1);
		tr($lang_details['row_hot_meter'], "
	<table><tr>
		<td class=\"embedded-head\" >" . $lang_details['text_views']."". $row["views"] . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td class=\"embedded-head\" >" . $lang_details['text_hits']. "" . $row["hits"] . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td class=\"embedded-head\">" .$lang_details['text_snatched'] . "<a href=\"viewsnatches.php?id=".$id."\">" . $row["times_completed"]. $lang_details['text_view_snatches'] . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td class=\"embedded-head\">" . $lang_details['row_last_seeder']. "" . gettime($row["last_action"]) . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		</tr>
		</table>",1);
...
//}
//print("<br /><br />");
//print ("<table style='border:1px solid #000000;'><tr><td class=\"text\" align=\"center\">".$lang_details['text_quick_comment']."<br /><br /><form id=\"compose\" name=\"comment\" method=\"post\" action=\"".htmlspecialchars("comment.php?action=add&type=torrent")."\" onsubmit=\"return postvalid(this);\"><input type=\"hidden\" name=\"pid\" value=\"".$id."\" /><br />");
